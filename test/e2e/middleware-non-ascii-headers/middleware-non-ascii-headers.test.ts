...
describe('Middleware Non-ASCII Headers', () => {
  ...
  // Test that App Route handlers can read headers correctly
  // ============================================================

  describe('App Route Handler', () => {
    it('should read ASCII headers correctly in route handler', async () => {
      const res = await fetchViaHTTP(
        next.url,
        '/api/route-headers',
        {},
      ...
      expect(body['x-simple']).toBe('hello-world')
      expect(body['x-number']).toBe('12345')
    })
